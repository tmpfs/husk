...
function run(parallel) {
  ...
  if(!parallel
    && this.children.length && (first instanceof ProcessStream)) {
...
function fd(num) {
  //console.log('set fd identifier: %s', num);
  //console.log('children %s', this.children.length);

  // last child is the previous command (reader)
  // at this point we don't know which command to pipe to.
  // this next command in the chain is not defined
