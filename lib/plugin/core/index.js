...
function run(parallel) {
  ...
  if(pipeline.length) {
    ...
    for(i = 1;i < pipeline.length;i++) {
      writer = pipeline[i];
      //console.log(i);
      //console.log((reader instanceof ProcessStream));
      //console.log((writer instanceof ProcessStream));
        //writer.once('end', function(){console.log('finished');})

      if(!(reader instanceof ProcessStream)
        && (writer instanceof ProcessStream)) {
        console.log('start next process on end..');
        reader.once('executed', function(){console.log('finished');})
      }
    ...
    writer.on('readable', function() {
      writer.removeAllListeners('readable');
      writer.end();
    })
  }
