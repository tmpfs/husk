...
function transform(chunk, encoding, cb) {
  ...
  // first exec in pipeline gobbles the empty string
  // when no data is provided it is new Buffer(0) to start
  // the stream transformations, but we don't want to push
  // this further downstream
  //if(!this.info.previous && ('' + chunk) === '') {
    //return cb();
  //}
