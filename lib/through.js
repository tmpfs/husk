...
/**
 *  Creates a passthrough, read, write or duplex stream.
 */
...
function duplexify(stream, reader, opts) {
  ...
  if(stream && stream.prototype) {
    if(!reader) {
      type.prototype._write = stream.prototype._write;
    }else{
      type.prototype._read = stream.prototype._read;
    }
  }
  return type;
}
 ...
 *  of node that to not support cork().
 */
function buffer() {
  return new (cork());
}
